...
$serebro = intval($ins_sum <= 0.01) ? ($serebro + ($serebro * 0.4) ) : ($serebro+($serebro * 0.1)); // Бонус после пополнения
   $add_tree = ( $ik_payment_amount >= 0.99) ? 0 : 0;
   $lsb = time();
   $to_referer = ($serebro * 0.10);
   
   $db->Query("UPDATE db_users_b SET money_b = money_b + '$serebro', а_t = e_t + '$add_tree', to_referer = to_referer + '$to_referer', last_sbor = '$lsb', insert_sum = insert_sum + '$ik_payment_amount' WHERE id = '{$user_id}'");
